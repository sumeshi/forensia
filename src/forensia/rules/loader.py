...
from forensia.rules.models import RulePackMetadata
    ...
    for pack_dir in sorted(rulepacks_dir.iterdir()):
        ...
        for path in pack_dir.rglob("*.yaml"):
            if "_schema" in path.parts or path.name == "pack.yaml":
                continue
            data = yaml.safe_load(path.read_text(encoding="utf-8"))
            if isinstance(data, dict) and not str(data.get("kind") or "").startswith("allowlist"):
                rule_id = data.get("id")
                if rule_id:
...
def detect_artifact_families(db) -> set[str]:
    ...
    if _has_table(db, "prefetch_executions"):
        ...
        if prefetch_like:
            clauses = " OR ".join("LOWER(executable_name) LIKE ?" for _ in prefetch_like)
        ...
        if email_prefetch:
            clauses = " OR ".join("LOWER(executable_name) LIKE ?" for _ in email_prefetch)
...
def resolve_active_packs(profile_path: str | Path | None, db, auto_rulepacks: bool = True) -> set[str]:
    ...
    if auto_rulepacks:
        ...
        if all_meta:
            ...
            for pack_id, meta in all_meta.items():
                if pack_id in packs:
                    continue
                required = meta.applies_when.get("artifact_families") if meta.applies_when else None
