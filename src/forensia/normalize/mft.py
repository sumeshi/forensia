...
def normalize_mft(case: Case, db: CaseDB) -> tuple[int, int]:
    ...
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            # mft2es standard format: record has "header" and "attributes" (keyed by type_code)
            ...
            # Expand SI and FN timestamps into mft_timeline rows
    ...
    return len(entry_rows), len(timeline_rows)
