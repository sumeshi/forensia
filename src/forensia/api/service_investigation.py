...
def list_evidence_sources_dto(db: CaseDB) -> list[EvidenceSourceDTO]:
    ...
    return [
        EvidenceSourceDTO(
            source_id=r[0], artifact_family=r[1], display_path=r[2],
            ingest_status=r[3], parser_name=r[4] or "", row_count=r[5] or 0,
            channel=r[6] or "", hosts=r[7] if isinstance(r[7], list) else [],
            min_time=r[8].isoformat() if r[8] else None,
            max_time=r[9].isoformat() if r[9] else None,
            error_code=r[10] or "", error_summary=r[11] or "",
        )
        for r in rows
    ]
...
def list_evidence_coverage_dto(db: CaseDB) -> list[EvidenceCoverageDTO]:
    ...
    return [
        EvidenceCoverageDTO(
            capability=r[0], host=r[1] or "", channel=r[2] or "",
            source_family=r[3], state=r[4], reason_code=r[5] or "",
...
def get_investigation_state_dto(db: CaseDB) -> InvestigationStateDTO | None:
    ...
    return InvestigationStateDTO(
        state_id=row[0], objective=row[1] or "", status=row[2] or "active",
        termination_policy=(
            row[3]
            if isinstance(row[3], dict)
            else json.loads(row[3]) if isinstance(row[3], str) else None
        ),
        stop_reason_code=row[4] or "", stop_reason=row[5] or "",
...
def list_report_gaps_dto(db: CaseDB, status: str | None = None) -> list[ReportGapDTO]:
    ...
    return [
        ReportGapDTO(
            gap_id=r[0], section_key=r[1] or "", block_heading=r[2] or "",
            description=r[3] or "", kind=r[4] or "", status=r[5] or "open",
            source_claim_id=r[6] or "", hypothesis_id=r[7] or "",
            task_id=r[8] or "", coverage_reason=r[9] or "",
...
def list_investigation_tasks_dto(db: CaseDB, status: str | None = None) -> list[InvestigationTaskDTO]:
    ...
    return [
        InvestigationTaskDTO(
            task_id=r[0], kind=r[1] or "", description=r[2] or "",
            status=r[3] or "open", gap_id=r[4] or "", hypothesis_id=r[5] or "",
            required_capability=r[6] or "", reason=r[7] or "",
...
def list_hypothesis_evidence_dto(db: CaseDB) -> list[HypothesisEvidenceLinkDTO]:
    ...
    return [
        HypothesisEvidenceLinkDTO(
            link_id=row[0], hypothesis_id=row[1], evidence_id=row[2],
            finding_id=row[3] or "", query_id=row[4] or "",
            assessment_id=row[5] or "", role=row[6] or "supporting",
            source_family=row[7] or "", derivation_group=row[8] or "",
