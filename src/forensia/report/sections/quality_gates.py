...
_QUALITY_CHECKS: list[QualityCheck] = [
...
def register_quality_check(
    check: QualityCheck, *, before: QualityCheck | None = None
) -> None:
    """Register one deterministic section quality check."""
    if check in _QUALITY_CHECKS:
        raise ValueError(f"quality check already registered: {check.__name__}")
    if before is None:
        _QUALITY_CHECKS.append(check)
        return
    try:
        index = _QUALITY_CHECKS.index(before)
    except ValueError as exc:
        raise ValueError(f"unknown insertion target: {before.__name__}") from exc
    _QUALITY_CHECKS.insert(index, check)
