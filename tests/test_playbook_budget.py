...
def test_filter_event_ids_reduces_size() -> None:
    ...
    filtered = _dfir_playbook("check", event_ids={4624, 4625, 4648, 4688, 4697, 7045, 1102, 104, 4768, 4771, 4720, 4724})
...
_SMALL_EVENT_IDS = {4624, 4625, 4648, 4688, 4697, 7045, 1102, 104, 4768, 4771, 4720, 4724}
...
def test_tables_gate_excludes_artifact_when_no_mft_prefetch() -> None:
    ...
    playbook = _dfir_playbook("check", event_ids=_SMALL_EVENT_IDS, tables={"evtx_events"})
...
def test_tables_gate_includes_artifact_when_mft_present() -> None:
    ...
    playbook = _dfir_playbook("check", event_ids=_SMALL_EVENT_IDS, tables={"evtx_events", "mft_entries"})
...
def test_tables_gate_ioc_with_mft() -> None:
    ...
    playbook = _dfir_playbook("check", event_ids=_SMALL_EVENT_IDS, tables={"prefetch_executions"})
...
def test_budget_enforcement_drops_sections_in_order(tmp_path: Any) -> None:
    ...
    try:
        import importlib
        import forensia.config
        importlib.reload(forensia.config)
...
def test_budget_drop_order_is_stable() -> None:
    ...
    expected = ["ioc", "app", "artifact", "extractor", "fp", "logon", "schema", "events"]
...
def test_table_gating_planning_phases() -> None:
    ...
    for phase in ("broad_plan", "hypothesis_plan"):
        playbook = _dfir_playbook(phase, event_ids=_SMALL_EVENT_IDS, tables={"mft_entries", "prefetch_executions"})
        assert "Artifact-to-Application Inference" not in playbook, f"{phase} should exclude artifact"
        assert "False-Positive Reduction Guidance" not in playbook, f"{phase} should exclude FP"
...
def test_combined_filtering_reaches_budget(tmp_path: Any) -> None:
    ...
    event_subset = {4624, 4625, 4648, 4688, 4697, 7045, 1102, 104, 4720, 4724, 4768, 4771, 4672, 4732, 4738, 5136, 5156, 5158}
...
def test_collect_event_ids_helper() -> None:
    ...
    from forensia.ai.prompts import _collect_event_ids
    results = [
