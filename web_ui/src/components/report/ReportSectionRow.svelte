...
<details class={`rounded-xl border ${section.is_highlighted ? "border-mocha-lavender" : section.is_writing ? "border-mocha-pink" : "border-mocha-surface1"} ${section.is_writing ? "ring-1 ring-mocha-pink" : ""} bg-mocha-surface0/90`} open={section.is_writing}>
  <summary class="flex cursor-pointer items-center justify-between gap-3 px-4 py-3">
    ...
    <div class="flex items-center gap-2 text-xs text-mocha-subtext0">
      ...
      <span class="text-mocha-overlay1">{section.gap_count} gaps</span>
    ...
    <div class="mb-2 flex items-center justify-between gap-3 text-xs text-mocha-subtext0">
      ...
      <div class="flex items-center gap-2">
        ...
        {#if section.status !== "human_reviewed"}
          <button class="rounded-md border border-mocha-green/40 px-2 py-1 text-mocha-green disabled:opacity-55" on:click={() => updateStatus("human_reviewed")} disabled={busy}>
            Mark reviewed
          </button>
        {:else}
          <button class="rounded-md border border-mocha-surface2 px-2 py-1 disabled:opacity-55" on:click={() => updateStatus("stable")} disabled={busy}>
            Reopen
          </button>
        {/if}
      </div>
