...
- **LLM trajectory and context control**: a session-scoped telemetry sink records logical decisions, every provider attempt, deterministic work, effective token limits, prompt-section sizes and retry/error lineage in `trace.duckdb`. Prompt builders send compact indexes and selected schema/recipe/event detail. Before a known context limit is crossed—or after an explicit provider context rejection—older turns are projected into a versioned regeneratable summary while recent turns and stable evidence/action references remain available for drill-down.
